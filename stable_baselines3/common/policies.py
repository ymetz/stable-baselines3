...
from stable_baselines3.common.torch_layers import BaseFeaturesExtractor, FlattenExtractor, MlpExtractor, NatureCNN, \
    create_mlp, TransformerExtractor, LSTM
    ...
    def predict(
        ...
        with th.no_grad():
            if state is not None:
                actions, state, distribution = self._predict(observation, state, deterministic=deterministic)
            else:
...
class ActorCriticTransformerPolicy(ActorCriticPolicy):
    ...
    def __init__(
        ...
        use_expln: bool = False,
        squash_output: bool = False,
        features_extractor_class: Type[BaseFeaturesExtractor] = TransformerExtractor,
        features_extractor_kwargs: Optional[Dict[str, Any]] = None,
...
class LstmPolicy(BasePolicy):
    """
    Assembled for imitation learning, not a full-fletched policy.
    Do not use for production or reinforcement learning.
    """
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        self.lstm = LSTM(self.features_dim, self.lstm_hidden_dim, batch_size= self.batch_size,orthogonal_init=self.ortho_init,
                         activation_fn=self.activation_fn, device="cuda")

        latent_dim_pi = self.lstm_hidden_dim
        ...
        self.value_net = nn.Linear(self.lstm_hidden_dim, 1)
        # Init weights: use orthogonal initialization
        # with small initial weight for the output
        if self.ortho_init:
            # TODO: check for features_extractor
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor, th.Tensor]:
        ...
        :return: action, value and log probability of the action
        """
        latent, state, latent_sde = self._get_latent(obs)
        # Evaluate the values for the given observations
        values = self.value_net(latent)
        distribution, pre_softmax_logits = self._get_action_dist_from_latent(latent, latent_sde=latent_sde)
        actions = distribution.get_actions(deterministic=deterministic)
        log_prob = distribution.log_prob(actions)
        return actions, state, values, log_prob, pre_softmax_logits
    ...
    def _get_latent(self, obs: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        # Preprocess the observation if needed
        features = self.extract_features(obs)
        features = th.unsqueeze(features, 0)
        latent = self.lstm(features)

        # Features for sde
        latent_sde = latent
        if self.sde_features_extractor is not None:
            latent_sde = self.sde_features_extractor(features)
        return latent, latent_sde
    ...
    def evaluate_actions(self, obs: th.Tensor, actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        :return: estimated value, log likelihood of taking those actions
            and entropy of the action distribution.
        """
        latent, latent_sde = self._get_latent(obs)
        distribution, _ = self._get_action_dist_from_latent(latent, latent_sde)
        log_prob = distribution.log_prob(actions)
        values = self.value_net(latent)
        return values, log_prob, distribution.entropy()

class ImitationLstmPolicy(LstmPolicy):
    ...
    def __init__(
        ...
        super(ImitationLstmPolicy, self).__init__(
            observation_space,
            action_space,
            lr_schedule,
            net_arch,
